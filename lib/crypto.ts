...
/**
 * Reversible AES-256-GCM encryption for secrets that must be recoverable in
 * plaintext — mailbox passwords (IMAP/SMTP need them to authenticate) and
 * Aadhaar numbers (a government ID that must not sit in the database in the
 * clear, but occasionally has to be read back).
 *
 * Stored format: base64( iv[12] | authTag[16] | ciphertext ).
 *
 * SECURITY: keep MAIL_ENCRYPTION_KEY out of the database and out of version
 * control. A leak of BOTH the DB and this key exposes everything encrypted here.
 * Changing the key makes existing values undecryptable.
 */
